...
newyorkcoin.auth('myusername', 'mypassword')

newyorkcoin.getDifficulty(function() {
...
```js
newyorkcoin.getNewAddress(function(err, address) {
...
```js
newyorkcoin.exec('getNewAddress')

newyorkcoin.exec('getbalance', function(err, balance) {
...
```js
newyorkcoin.set('host', '127.0.0.1')
```
...
var newyorkcoin = require('newyorkcoin')({
    user:'user'
})

newyorkcoin.set('pass', 'somn')
newyorkcoin.set({port:18823})
...
You may pass an optional function `passphrasecallback` to the `node-newyorkcoin` initialization function to manage wallet unlocks. `passphrasecallback` should be a function accepting three arguments:
...
By default `newyorkcoind` exposes its JSON-RPC interface via HTTP; that is, all RPC commands are transmitted in plain text across the network! To secure the JSON-RPC channel you can supply `newyorkcoind` with a self-signed SSL certificate and an associated private key to enable HTTPS. For example, in your `newyorkcoin.conf`:

    rpcssl=1
    rpcsslcertificatechainfile=/etc/ssl/certs/newyorkcoind.crt
    rpcsslprivatekeyfile=/etc/ssl/private/newyorkcoind.pem

In order to securely access an SSL encrypted JSON-RPC interface you need a copy of the self-signed certificate from the server: in this case `newyorkcoind.crt`. Pass your self-signed certificate in the `ca` option and set `https: true` and node-newyorkcoin is secured!
...
nodeunit test/test-node-newyorkcoin.js
```
